...
public class OrderServlet extends BaseServlet {
	...
	public void delOrder(HttpServletRequest request, HttpServletResponse response)
		...
		//根据orderId找出用户id
		List<Map<String, Object>> userId = service.getById(orderid);
		//根据orderId找出订单钱款
		List<Map<String, Object>> num = service.getPriceById(orderid);
		//根据order找出用户账号返还钱款
		Gms_User gms_user = userService.getById((String) userId.get(0).get("order_user_id"));
		gms_user.setBalance(gms_user.getBalance() + Double.parseDouble(num.get(0).get("order_price").toString()));
		userService.updateBalance(gms_user);
		...
		request.getSession().setAttribute("user", gms_user);
		//******************改变分时状态为已预约************************
...
//		System.out.println(pageBean);
	...
	public void submitOrder(HttpServletRequest request, HttpServletResponse response)
		...
		Gms_User user = (Gms_User) session.getAttribute("user");
		//查询数据库该用户余额
		Gms_User userDetail = userService.getById(user.getUser_Id());
		user.setBalance(userDetail.getBalance());
		//如果为校外用户
		if (user.getIs_Student().equals("0")) {
			if ((userDetail.getBalance() < orderlist.getOrder_Price()) ) {
				session.removeAttribute("orderlist");
				response.sendRedirect(request.getContextPath() + "/user/payFail.jsp");
				return;
			}
			//扣减余额
			user.setBalance(user.getBalance() - orderlist.getOrder_Price());
		...
		Date order_Date = orderlist.getOrder_Date();
