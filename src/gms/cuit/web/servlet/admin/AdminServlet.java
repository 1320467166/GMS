...
//        adminService.update_venue(venue);
        adminService.update_user(userId,balance);
        String json = "{}";
     ...
     * @throws SQLException
     */
    public void query_user(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException, SQLException {
        int currentPage = 1;
        int currentCount = 5;
...
//        PageBean<Gms_Venue> pageBean_venue = adminService.query_venueByKey(currentPage, currentCount, query_key);
        PageBean<Gms_User> pageBean_user = adminService.query_userByKey(currentPage, currentCount, query_key);
        request.setAttribute("pageBean_user", pageBean_user);
        request.setAttribute("query_key", query_key);
        request.getRequestDispatcher("admin/bck_user.jsp").forward(request, response);
    }
    /**
     * 用户登录
     * @param request
...
//                Object order_date = mp.get("order_date");
//                java.time.LocalDateTime order_mktime = (java.time.LocalDateTime) mp.get("order_mktime");
//                mp.remove("order_date");
//                mp.remove("order_mktime");
//                orderitem.setOrder_date((Date) order_date);
//                orderitem.setOrder_Mktime((Date) DateUtils.asDate(order_mktime));
