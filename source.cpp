...
//        " "<<venue[idc].capacity<<" "<<venue[idc].open<<" "<<venue[idc].close<<" "<<venue[idc].is_del<<endl;
        ...
        for(int j=0;j<4;j++){
            venue[idc].vd[j].date="2023-2-"+toString(prdate+j);
...
void getorder(){
    for(int i=0;i<orderc;i++){
        ...
        order[i].state=(rand()%5)>0?0:1;//�󲿷��ǽ�����
